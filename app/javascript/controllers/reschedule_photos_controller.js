...
/**
 * Controller para captura y upload de fotos de reprogramación
...
export default class extends PhotoBaseController {
  async connect() {
    // Configurar valores específicos para fotos de reprogramación
    this.storeNameValue = 'reschedule-photos'
    this.inputNameValue = 'reschedule_photos[]'
    this.modalIdValue = 'reschedule-photo-modal'  // ✅ FIXED: Modal único
  ...
  async uploadPhotos() {
    // VALIDACIÓN ESPECÍFICA PARA REPROGRAMACIÓN: Verificar motivo
    const reasonInput = document.getElementById('reason')
    if (!reasonInput || !reasonInput.value || reasonInput.value.trim() === '') {
      // Mostrar alerta estilo flash
      this.showFlashAlert('Se requiere un motivo para reprogramar Por favor.', 'error')
